    ...
    async function fetchAndRenderApps() {
        ...
        data.forEach(app => {
            ...
            listItem.innerHTML = `
                <span>${app.name}</span>
                <div>
                    <button class="btn btn-sm btn-secondary me-2" data-bs-toggle="modal" data-bs-target="#editAppModal" onclick="populateEditForm('${app.id}', '${app.name}', '${app.description}', '${app.link}', '${app.icon}')">
